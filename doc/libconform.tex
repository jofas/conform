...
\begin{align*}
  ...
  \Rbag, z_i) = \Delta\big(y,D_{\Lbag z_1,\dots,z_{i-1},
...
$b_{n+1} = b_i > 0$
The p-value can only change at $u_i$'s or $v_i$'s.
Therefore all $u_i$'s and $v_i$'s are sorted in ascending
...
only for single $s_j$'s: $S_i: s_j \in S_i, i=1,\dots,n$.
...
providing it with its $a_i$'s and $b_i$'s. Currently the
library provides one of these regression algorithms, based
on the $k$-nearest neighbor method. For $b_i, i=1,\dots,n$
it returns 0, for $b_{n+1}$ it returns 1. $a_i$ is the
difference between $y_i$ and the average of the labels of
its $k$-nearest neighbors. $y_{n+1}$ is set to 0 therefore
the $k$-nearest neighbor method returns the negated average
of the labels of the $k$-nearest neighbors of $x_{n+1}$ as
$a_{n+1}$.
